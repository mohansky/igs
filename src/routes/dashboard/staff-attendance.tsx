      ...
      {loaded && staff.length > 0 && (
        <>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Badge variant="success">{summary.leave} leave</Badge>
          </div>
                ...
                <TableRow key={s.userId}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button
                        variant={s.status === 'leave' ? 'success' : 'ghost'}
                        size="sm"
...
function StaffAttendanceHistory() {
  const [filteredRows, setFilteredRows] = useState<StaffAttendanceRecord[]>([])

  const { data: records = [], isLoading } = useQuery({
    queryKey: ['staff-attendance', 'all'],
    queryFn: () =>
      listStaffAttendance({ data: {} }) as Promise<StaffAttendanceRecord[]>,
  })
  ...
  const staffOptions = useMemo(() => {
    ...
    for (const r of records) if (r.staffName) names.add(r.staffName)
    return [
  ...
  const summaryBar = (
    <div className="mb-2 flex flex-wrap items-center gap-2">
      ...
      <Badge variant="success">Leave: {summary.leave}</Badge>
    </div>
  )

...
function PreferencesSection() {
  ...
  const [landingPath, setLandingPathState] = useState('/dashboard')
  ...
  useEffect(() => {
    const sync = () => {
      ...
      setSidebarOpenState(getSidebarOpen())
      setLandingPathState(getLandingPath())
    }
    sync()
    return subscribeToPreferences(sync)
  }, [])
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Select
              value={theme}
              onValueChange={(v) => setThemeMode(String(v) as ThemeMode)}
            >
          ...
          <div className="space-y-2">
            <Label htmlFor="pref-rows">Default rows per table</Label>
            <Select
              value={String(rows)}
              onValueChange={(v) => setRowsPref(Number(v))}
            >
              <SelectTrigger id="pref-rows" className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {[10, 20, 50, 100].map((n) => (
            ...
            <p className="text-xs text-muted-foreground">
              Used for tables without a saved page size.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={density}
              onValueChange={(v) => setDensity(String(v) as Density)}
            >
              <SelectTrigger id="pref-density" className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={dateFormat}
              onValueChange={(v) => setDateFormat(String(v) as DateFormat)}
            >
              <SelectTrigger id="pref-date" className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <p className="text-xs text-muted-foreground">
              Preview: {formatDate(today)}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="pref-landing">Landing page after login</Label>
            ...
            <p className="text-xs text-muted-foreground">
              Dashboard page to open when you sign in.
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="pref-sidebar">Sidebar</Label>
            <div className="flex h-9 items-center gap-3">
              ...
              <span className="text-sm">
                {sidebarOpen ? 'Expanded by default' : 'Collapsed by default'}
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              Applies when you first open the dashboard.
            </p>
          </div>
        </div>
      </CardContent>
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <Label>Role</Label>
          <div>
            <Badge variant={getRoleVariant(userRole)} className="capitalize">
              {userRole}
            </Badge>
          </div>
        </div>
